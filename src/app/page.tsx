...
export default function Home() {
  ...
  const handleEditSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!editTarget) return;
    setTransactions((prev) => prev.map((t) => t.id === editTarget.id ? { ...editTarget, amount: parseFloat(editTarget.amount as any) } : t));
  ...
  function generateDemoTransactions() {
    ...
    for (let m = 7; m <= 12; m++) { // Jul-Dec 2024
      for (let i = 0; i < 10; i++) {
        const seed = m * 100 + i;
        // More income in Dec/Jan, more expenses in summer
        let type: "income" | "expense" = seededRandom(seed) > (m === 12 || m === 1 ? 0.5 : 0.3) ? "expense" : "income";
        ...
        const category = cats[Math.floor(seededRandom(seed + 3) * cats.length)];
  ...
  const pieData = categories.map(cat => {
    const value = pieTx.filter(t => t.category === cat).reduce((sum, t) => sum + t.amount, 0);
              ...
              <div className="bg-gradient-to-r from-blue-100 via-white to-pink-100 rounded-2xl shadow-xl p-8 flex flex-col items-center animate-fadeIn">
                ...
                <ResponsiveContainer width="100%" height={360}>
                  ...
                  >
                    ...
                    <Tooltip
                      formatter={(v: number, name: string, props: any) => [`$${v.toLocaleString(undefined, {maximumFractionDigits:2})}`, name]}
