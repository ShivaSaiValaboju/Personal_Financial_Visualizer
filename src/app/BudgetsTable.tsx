...
export function BudgetsTable({ categories, transactions, currentMonth, currentYear, getCategoryColor }: BudgetsTableProps) {
  ...
  categories.forEach(cat => {
    actuals[cat] = transactions.filter(t => t.type === 'expense' && t.category === cat && new Date(t.date).getMonth() + 1 === currentMonth && new Date(t.date).getFullYear() === currentYear).reduce((sum, t) => sum + t.amount, 0);
  });
